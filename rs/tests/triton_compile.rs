...
"#;
    assert!(compile_triton(source, "test.tri").is_ok());
}
...
"#;
    let tasm = compile_triton(source, "events.tri").expect("events program should compile");
...
fn test_error_max_nesting_depth() {
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            let result = trident::parse_source_silent(&src, "t.tri");
            assert!(
                result.is_err(),
...
fn test_coin_compiles() {
    let path = trident_repo_path("os/neptune/standards/coin.tri");
    if !path.exists() {
        return;
    }
    let tasm = compile_project_triton(&path).expect("coin program should compile");
...
fn test_card_compiles() {
    let path = trident_repo_path("os/neptune/standards/card.tri");
    if !path.exists() {
        return;
    }
    let tasm = compile_project_triton(&path).expect("card program should compile");
