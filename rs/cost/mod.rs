...
/// Compute the AET-table cost of a single Trident source file.
///
/// Single-file only: trident does not expose its multi-module resolver
/// outside the compiler (`PreparedProject` is `pub(crate)` there), so this
/// cannot follow `use` imports the way trident's own `analyze_costs_project`
/// used to before the move. `trisha build --costs` reads the entry file as
/// one string, which is exact for single-file programs.
pub fn analyze_source(source: &str, filename: &str) -> Result<ProgramCost, String> {
    let file = trident::parse_source_silent(source, filename).map_err(|errs| {
        errs.iter()
            .map(|d| d.message.clone())
            .collect::<Vec<_>>()
            .join("; ")
    })?;
    Ok(CostAnalyzer::default().analyze_file(&file))
}
